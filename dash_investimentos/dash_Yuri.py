...
with st.sidebar:
    st.header("⚙️ Parâmetros do Investimento")
    
    initial_investment_input = st.number_input(
        "Valor do Investimento Inicial (R$)",
    ...
    is_consortium = st.checkbox("O valor investido é de consórcio?")
    consortium_interest_rate_input = 0
    if is_consortium:
        consortium_interest_rate_input = st.number_input(
            "Juros anuais do consórcio (%)",
            min_value=0.0, max_value=20.0, value=4.5, step=0.1,
            format="%.1f",
            help="Taxa de juros anual a ser paga sobre o valor do consórcio."
        )
    ...
    if use_m2_pricing:
        st.markdown("---")
        area_terreno_m2 = st.number_input("Área do Terreno (m²)", min_value=1.0, value=720.0, step=10.0)
        area_construcao_m2 = st.number_input("Área de Construção (m²)", min_value=1.0, value=450.0, step=10.0)
        st.markdown("---")
        
        land_cost_per_m2 = st.number_input("Valor do m² do Terreno (R$)", min_value=0, value=1528, step=50)
        construction_cost_per_m2 = st.number_input("Valor do m² da Construção (R$)", min_value=0, value=4889, step=100)
        sale_price_per_m2 = st.number_input("Valor do m² de Venda (R$)", min_value=0, value=10000, step=100)
    ...
    construction_years_input = st.slider(
        "Tempo de Construção (anos)",
        min_value=1.0, max_value=5.0, value=1.5, step=0.5,
        format="%.1f anos",
        help="Selecione o período total para a construção e venda da casa."
    )
    
    st.markdown("---")
    st.header("💰 Parâmetros Fiscais e Financeiros")
...
months_input = int(construction_years_input * 12)
...
consortium_cost = 0
if is_consortium:
    consortium_cost = initial_investment_input * (consortium_interest_rate_input / 100) * construction_years_input
...
with col_rf2:
    ...
    st.metric("💸 Imposto de Renda (15%)", format_currency(tax_s1), help="15% sobre o lucro da aplicação.")
...
# Linha 2: Impostos e outros custos
cols_costs_2 = st.columns(3)
with cols_costs_2[0]:
    st.metric("Imposto Ganho Capital", format_currency(tax_details['Imposto Pago (Ganho de Capital)']), help="Calculado sobre o lucro da venda do imóvel.")
with cols_costs_2[1]:
    st.metric("IR sobre Rendimentos (15%)", format_currency(tax_s2_income), help="15% sobre o lucro do fundo da obra e do capital excedente.")
if is_consortium:
    with cols_costs_2[2]:
        st.metric("Custo do Consórcio", format_currency(consortium_cost), help=f"Juros de {consortium_interest_rate_input}% a.a. por {construction_years_input} anos.")
...
final_s2_total_benefit = final_s2 + tax_details["Economia de Imposto (Empresa)"] - consortium_cost
...
with st.expander("📄 Gerar Relatório para Impressão/PDF"):
    ...
    if is_consortium:
        st.write(f"- **Financiamento via Consórcio com Juros de:** {consortium_interest_rate_input}% a.a.")
